    ...
    <div className='w-full'>
      <div className='flex flex-col gap-3'>
        ...
        <div className='relative overflow-hidden ' ref={containerRef}>
          ...
          >
            {items.map((item) => (
              <div key={item.id} className='relative shrink-0 w-full h-[100vh] group'>
                <img
                  src={item.url}
                  alt={item.title}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8 md:p-12">
                    <div className="transform translate-y-8 group-hover:translate-y-0 transition-transform duration-500 ease-out">
                        <h3 className="text-2xl md:text-4xl font-light text-white tracking-[0.2em] mb-2 uppercase">{item.title}</h3>
                        <p className="text-sm md:text-base text-gray-300 font-light max-w-lg">
                            Captured in the wild, this moment reflects the raw beauty of nature. Every detail tells a story of survival and grace.
                        </p>
                    </div>
                </div>
              </div>
          ...
          <motion.button
            ...
            className={`absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full flex items-center justify-center shadow-lg transition-transform z-10 border border-white/20
              ${
                index === 0
                  ? 'bg-black/20 opacity-40 cursor-not-allowed text-white/40'
                  : 'bg-black/50 backdrop-blur-md text-white hover:scale-110 hover:bg-black/70'
              }`}
          >
            <svg
              className='w-6 h-6'
          ...
          <motion.button
            ...
            className={`absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full flex items-center justify-center shadow-lg transition-transform z-10 border border-white/20
              ${
